...
public class DiskManager {
	...
	public static String tryToFetchCachePathByTypeBinding(DiskCacheType type) {
		...
		case DOWNLOADED_APK:
			retDir = retDir
					+ (UtilsRuntime.isExternalStorageAvailable() ? "downloaded_files/"
							: "/");
			break;
		case TMP:
	...
	private static void tryToCleanDiskUnderDir(String fullPath, long maxSize) {
		UtilsConfig.LOGD("[[tryToCleanDiskUnderDir]] try to clean path : "
				+ fullPath + " }}}}}}}} =================");
		...
		if (!TextUtils.isEmpty(fullPath)) {
			...
			if (cleanDir.exists()) {
				...
				if (DEBUG) {
					UtilsConfig.LOGD("          current disk Size = "
							+ FileUtil.convertStorage(diskSize)
							+ " max size = "
							+ FileUtil.convertStorage(maxSize)
							+ " current time = "
							+ UtilsRuntime.debugFormatTime(System
									.currentTimeMillis()));
				}
				...
				if (diskSize >= maxSize) {
					...
					UtilsConfig.LOGD(" should delete size = "
	...
	public static void tryToCleanDisk() {
		...
		tryToCleanDiskUnderDir(UtilsConfig.DEVICE_INFO.flashDataPath,
				MAX_FLASH_SIZE);
	}
