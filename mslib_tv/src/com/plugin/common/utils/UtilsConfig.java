...
public class UtilsConfig {
		...
		if (context != null) {
			...
			if (UtilsRuntime.isExternalStorageAvailable()) {
				diskDir = Environment.getExternalStorageDirectory()
						.getAbsolutePath();
				CURRENT_PACKAGE_NAME_BINDING = UtilsRuntime
			...
			DiskManager.init();
            BitmapDiskTools.init(DiskManager.tryToFetchCachePathByTypeBinding(DiskManager.DiskCacheType.PICTURE));

			SingleInstanceManager.getInstance().init(
					context.getApplicationContext());
			UtilsConfig.DEVICE_INFO = new DeviceInfo(
					context.getApplicationContext());
		} else {
	...
	public static void LOGD(String tag, String msg) {
		String method = UtilsRuntime.getCurrentStackMethodName();
		DebugLog.d(tag, "[[" + method + "]]" + msg);
	}
